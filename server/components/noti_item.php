...
<?php
    ...
    if(isset($_POST['noti_input']))
    {
        ...
        {
            $sql = "SELECT * FROM repair_form ORDER BY create_at DESC";
            $rsult = $connection->query($sql);
        ...
        else if($noti_input == "event-created")
        {
        ...
        while($data= $data = mysqli_fetch_all($rsult, MYSQLI_ASSOC))
        {
            for($i =0; $i<count($data); $i++)
            {
                ...
                <div class='notification-list_creation-time'>
                    ...
                    echo  "</small></p>  
                    <form class='event-delete_submit'>
                        <input name='id-event_delete' value='".$data[$i]['id']."' style='display:none;'/>
                        <button type='submit'>Xóa</button>
                    </form>
                  
                    <form class='edit-event_form'>
                        <input name='event-id' value='".$data[$i]['id']."' style='display:none'/>
                        <input name='title-sent' value='".$data[$i]['title']."' style='display:none'/>
                        <input name='content-sent' value='".$data[$i]['content']."' style='display:none'/>
                        <button class='edit-event_btn'>Sửa</button>
                    </form>
                </div>
        ...
        else if($noti_input ==  'news-created')
        {
            $sql = "SELECT * FROM news ORDER BY create_at DESC";
            $rsult = $connection->query($sql);
            echo 
            "
                <button class='new-news'>
                    <i class='bx bx-plus'></i>
                    Tạo mới
                </button>
            ";
        while($data= $data = mysqli_fetch_all($rsult, MYSQLI_ASSOC))
        {
            for($i =0; $i<count($data); $i++)
            {
                ...
                <div class='notification-list_content'>
                    <div class='notification-list_img'>
                        <img src='../news_images/".$data[$i]['image_url']."' alt='Feature image' class='event_image'>
                    </div>
                    <div class='notification-list_detail'>
                        <div class='title'>
                ...
                <div class='notification-list_creation-time'>
                    ...
                    echo  "</small></p>  
                    <form class='new-delete_submit'>
                        <input name='id-new_delete' value='".$data[$i]['id']."' style='display:none;'/>
                        <button type='submit'>Xóa</button>
                    </form>
                        <form class='edit-new_form'>
                            <input name='new-id' value='".$data[$i]['id']."' style='display:none'/>
                            <input name='title-sent' value='".$data[$i]['title']."' style='display:none'/>
                            <input name='content-sent' value='".$data[$i]['content']."' style='display:none'/>
                            <button class='edit-new_btn'>Sửa</button>
                        </form>
                </div>
        ...
        {
            $sql = "SELECT * FROM expenditure ORDER BY create_at DESC";
            $currentDate = date('Y-m-d');   
            $rsult = $connection->query($sql);
...
//   });
  $(document).ready(function() {
    ...
    document.querySelectorAll('.edit-new_form').forEach(item=>{
     item.onsubmit = function(e) {
         e.preventDefault();     
         const newId=item.querySelector('.edit-new_form input[name="new-id"]');
         const titleSent=item.querySelector('.edit-new_form input[name="title-sent"]');
         const contentSent=item.querySelector('.edit-new_form input[name="content-sent"]');
         var formData= {};
         formData['new-id']= newId.value;
         formData['title-sent'] = titleSent.value;
         formData['content-sent']=contentSent.value;
         formData['record-type'] = "news";
         console.log(formData);
         $.ajax({
             // type: form.attr('method'),
             url: "./components/new_record.php", 
             method:"POST",
             data: formData,
             success: function(response) {
                 $('.overlay-wrapper2 .overlay-content').html(response)
 
             }
         });
     }
 })
 document.querySelectorAll('.edit-event_form').forEach(item=>{
    console.log(item);
     item.onsubmit = function(e) {
         e.preventDefault();     
         const eventId=item.querySelector('.edit-event_form input[name="event-id"]');
         const titleSent=item.querySelector('.edit-event_form input[name="title-sent"]');
         const contentSent=item.querySelector('.edit-event_form input[name="content-sent"]');
         var formData= {};
         formData['event-id']= eventId.value;
         formData['title-sent'] = titleSent.value;
         formData['content-sent']=contentSent.value;
         formData['record-type'] = "event";
         console.log(formData);
         $.ajax({
             url: "./components/new_record.php", 
             method:"POST",
    ...
    $('.event-delete_submit').submit(function(e) {
        e.preventDefault();     
        var form = $(this);
        var formData = form.serialize(); 
        console.log($('.notification-ui_dd-content'))   
        $.ajax({
            type: form.attr('method'),
            url: "./components/get_data.php", 
            method:"POST",
    ...
    $('.new-delete_submit').submit(function(e) {
        e.preventDefault();     
        var form = $(this);
        var formData = form.serialize(); 
        // console.log(formData);
